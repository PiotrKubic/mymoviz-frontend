...
const backend_URL = "https://mymoviz-backend-7gq8.vercel.app";
...
function Home() {
  ...
  useEffect(() => {
    async function fetchMoviesData() {
      ...
      moviesDataformated = moviesDataformated.movies.map((data) => {
        ...
        return {
          title: data.title,
          poster: `${TMDBBaseImgURL}${posterSize}${data.poster_path}`,
          voteAverage: data.vote_average,
          voteCount: data.vote_count,
          overview: overview,
        };
      });
      setMoviesData(moviesDataformated);
    }
    fetchMoviesData();
  }, []);
